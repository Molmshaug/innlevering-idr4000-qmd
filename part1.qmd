...
Testleder og assistent gjorde klar biosen, neseklypen, kalibrerte og satt samme munnstykket før testen. Før teststart ble forsøkspersoners navn, alder, fødselsdato, kjønn, høyde og vekt (med minst mulig tøy, 300g trekt fra) lagt inn på dataen. Deretter ble sykkelen (Sport Excalibur) tilpasset forsøkspersonene og byttet til riktig pedal i forhold til sykkelskoene. Forsøkspersonene ble informert om gjennomføringen av testen samt BORG skalaen som ble tatt i bruk under testen. Testen ble gjennomført på Sport Excalibur uten oppvarming. Hvert drag varte i 5 min og ved 4:30 ble forsøkspersonene spurt hvor på BORG skalaen personen befinner seg og det ble tatt laktat ved stikk i fingeren.
...
Datasettet ble logget for hånd og tastet inn i Excel. Datasettet er deretter lastet inn fra Excel til Rstudio ved bruk av readxl-pakken i Rstudio. Alle statistiske analyser er gjort i Rstudio (versjon 2022.07.2.576) for windows. 
...
library(readxl)
library(gt)
lac_data <- read_excel("data/Part.1/lac_data.xlsx")
View(lac_data)
```
...
Kjønnsfordelingen mellom kvinner og menn var jevn, med 4 kvinner og 5 menn. Alderspennet var fra 23 til 31 år, med en gjennomsnittsalder på 25.6 år (±2.4 år).  
...
lac_data <- read_excel("data/Part.1/lac_data.xlsx", na = "NA")
View(lac_data)
...
lac_data %>%
  ...
  tab_footnote("Standard deviation in parentheses")
```


### Gjennomsnitt ved 4mmol/L

Gjennomsnitt av resultatene ved 4mmol laktat. Laktat eller melkesyre i blodet refereres til mangel på oksygentilførsel til deler av kroppen under arbeid [@husoy_2022]. Laktatterskel kan eksempelvis forklare hvilken intensitet du kan holde over en gitt periode, og unngå utmattelse.
...
gt(caption = "Subjects") %>%
  cols_label(time = "Timepoint",
             ...
             sdHF_4mmol = "",
             n = "Participants") %>% 
  tab_header("Table 2: means at 4mmol/L") %>% 
  tab_footnote("Standard deviation in parentheses")
```
...
Watt produsert på terskel (4mmol/L viser fordeling basert på deltaker og kjønn, ved pre- og posttest.
```{r}
#| message: false
#| include: true
#| warning: false
...
lac_data %>% 
  ...
  ggplot(aes(y = watt_4mmol, 
             ...
             color = sex)) + 
  geom_point() + theme_bw() +
  labs(title = "Table 3: Watt at 4mmol/L",
       y = "Watt",
       x = "Tidpunkt",
       color = "Kjønn",
       caption = "Viser watt på terskel (4mmol/L) ved pre- og posttest \nMenn markert i blått, kvinner markert i rødt")
```
...
#| echo: false
  ...
  summarise(hf.m = mean(c(HF_4mmol_pre, HF_4mmol_post), na.rm = TRUE),
...
Vi har tatt i bruk metodene fra @hopkins2000 for å måle reliabiliteten. Den ene metoden vi har brukt kalles «within-subject variation». Denne metoden omhandler tilfeldige variasjoner og det finnes statistikk som fanger opp disse tilfeldige variasjonene av enkeltpersoners resultater i en retestet test. Dette omtales ofte som standardavvik og representerer typical error, i en måling [@hopkins2000]. Utregningen av typical error gjøres ved å dele standardavviket på kvadratroten av 2 [@hopkins2000]. Det finnes ulike typer kilder til typical error i en måling, derav biologiske faktorer. Forsøkspersonene kan ha ulik fysisk- og mental tilstand under de ulike testene som kan være med på å påvirke resultatet. Hopkins påpeker også at det er ideelt at det i en test er mange forsøkspersoner og få prøver/re-tester [@hopkins2000].
